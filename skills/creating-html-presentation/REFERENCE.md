...
| **Multiple Choice / Cross Out** | `split_table` | Native HTML `<table>` with context-aware icons. **No bullets/carats**. |
| **Discussion Questions** | `split_table` | Native HTML `<table>` with icons. **No `<ul>` bullets**. |
...
```json
{
    ...
    "example": "Don't drop that vase! It's very <span style='color: #FFD700;'>precious</span> to my grandmother.",
    "notes": "Focus on the value of objects."
}
```
